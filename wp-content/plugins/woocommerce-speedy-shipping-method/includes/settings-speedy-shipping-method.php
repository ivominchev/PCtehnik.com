...
if ( $this->get_option('allowed_methods') ) {
...
foreach ($clients as $client) {
	$contact_clients[$client['clientId']] = sprintf(__( 'ID: %s, Име: %s, Адрес: %s', SPEEDY_TEXT_DOMAIN ), $client['clientId'], $client['name'], $client['address']);
}
...
$this->form_fields = array(
	...
	'option_before_payment' => array(
		...
		'options'       => array(
			...
			'test'       =>  __( 'Тествай преди да платиш', SPEEDY_TEXT_DOMAIN ),
			'open'       =>  __( 'Отвори преди да платиш', SPEEDY_TEXT_DOMAIN ),
		),
	),
	'return_payer_type' => array(
		'title'         => __( 'Опции преди плащане - Платец на товарителница за връщане:', SPEEDY_TEXT_DOMAIN ),
		'type'          => 'select',
	...
	'return_package_city_service_id' => array(
		'title'         => __( 'Опции преди плащане - Услуга на товарителница за връщане (градска услуга):', SPEEDY_TEXT_DOMAIN ),
		'type'          => 'select',
		'class'         => ($this->get_option('option_before_payment') == 'no_option') ? 'hidden-row' : 'wc-enhanced-select',
		'options'       => $services_select,
	),
	'return_package_intercity_service_id' => array(
		'title'         => __( 'Опции преди плащане - Услуга на товарителница за връщане (междуградска услуга):', SPEEDY_TEXT_DOMAIN ),
		'type'          => 'select',
		'class'         => ($this->get_option('option_before_payment') == 'no_option') ? 'hidden-row' : 'wc-enhanced-select',
		'options'       => $services_select,
	),
	...
	'return_voucher_city_service_id' => array(
		'title'         => __( 'Ваучер за връщане - градска услуга:', SPEEDY_TEXT_DOMAIN ),
		'type'          => 'select',
		'class'   => (!$this->get_option('return_voucher')) ? 'hidden-row' : 'wc-enhanced-select',
		'options'     => $services_select,
	),
	'return_voucher_intercity_service_id' => array(
		'title'         => __( 'Ваучер за връщане - междуградска услуга:', SPEEDY_TEXT_DOMAIN ),
		'type'          => 'select',
		'class'   => (!$this->get_option('return_voucher')) ? 'hidden-row' : 'wc-enhanced-select',
		'options'     => $services_select,
	),
	'return_voucher_payer_type' => array(
		'title'         => __( 'Ваучер за връщане - страна платец:', SPEEDY_TEXT_DOMAIN ),
		'type'          => 'select',
		'class'   => (!$this->get_option('return_voucher')) ? 'hidden-row' : 'wc-enhanced-select',
		'options'       => array(
