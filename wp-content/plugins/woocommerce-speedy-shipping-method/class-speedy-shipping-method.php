...
class WC_Speedy_Shipping_Method extends WC_Shipping_Method {
	...
	public function speedy_process_admin_options() {
		...
		$this->addWeightDimensions( $_POST['weight_dimensions'] );
		unset($_POST['weight_dimensions']);
	...
	static function speedy_add_form() {
		...
		if ($chosen_shipping == $wc_speedy_shipping_method->id) {
			...
			if ( isset( $speedy_data['shipping_method_id'] ) ) {
				$results_speedy = $wc_speedy_shipping_method->getQuotePublic(array('validate' => true), $speedy_data['shipping_method_id']);
			...
			if (isset($speedy_data['fixed_time_cb'])) {
				$data['fixed_time_cb'] = $speedy_data['fixed_time_cb'];
			} else {
				$data['fixed_time_cb'] = false;
			}

			if (!isset($speedy_data['methods_fixed_time'])) {
				$data['fixed_time_cb'] = false;
			...
			if (!$data['country_id']) {
				$countries = $wc_speedy_shipping_method->speedy->getCountries( array( 'iso_code_2' => $woocommerce->customer->get_shipping_country() ), $lang );
				...
				if (!$wc_speedy_shipping_method->speedy->getError()) {
					if (count($countries) == 1) {
						...
						$states = $wc_speedy_shipping_method->speedy->getStates( $country['id'], $woocommerce->customer->get_shipping_state(), $lang );
						if (!$wc_speedy_shipping_method->speedy->getError()) {
							if (count($states) == 1) {
								$data['state'] = $states[0]['name'];
								$data['state_id'] = $states[0]['id'];

								$data['state_disabled'] = true;
							} else {
								foreach ($states as $state) {
									if ($woocommerce->customer->get_shipping_country() == $state['code']) {
				...
				$countries = $wc_speedy_shipping_method->speedy->getCountries( array( 'country_id' => $data['country_id'] ), $lang );
				if (count($countries) == 1) {
					$country = $countries[0];
					$data['active_currency_code'] = $country['active_currency_code'];
					if (!$country['active_currency_code']) {
			...
			if ($data['city_id']) {
				$data['offices'] = $wc_speedy_shipping_method->speedy->getOffices( null, $data['city_id'], $lang );
	...
	static function speedy_submit_form() {
		...
		$errors = $speedy_shipping_method->validateSpeedyForm(); // If errors exits
		...
		if ( $_SERVER['REQUEST_METHOD'] == 'POST' && empty( $errors ) ) {
			...
			if ( WC()->session->speedy ) {
				WC()->session->speedy = array_merge(WC()->session->speedy, $_POST);
			} else {
				WC()->session->set( 'speedy', $_POST);
			}

			$results_speedy = $speedy_shipping_method->getQuotePublic(array('validate' => true), $results['shipping_method_id']);
	 ...
	 * Generate speedy methods
	 */
	public function set_speedy_method() {
		global $woocommerce;
		...
		if (isset($_POST['method_id']) && isset($_POST['method_price'])) {
			...
			if ( ! $speedy_shipping_method->invoice_courrier_sevice_as_text ) {
				...
				if ( isset( $speedy_session['cod'] ) && $speedy_session['cod'] && in_array( $this->pricing, $allowed_pricings )) {
	...
	function generate_currency_rate_html( $key, $data ) {
		...
		<tr valign="top" id="add_rate">
			...
			<td class="forminp">
				<button type="button" class="button" onclick="addRate();">Add</button>
			</td>
		</tr>
				...
				<table id="weight_dimensions">
					...
					<?php foreach($weight_dimensions['options'] as $key => $dimention) { ?>
						<tr data-row="<?php echo $key; ?>">
							<td>
								<input type="text" name="weight_dimensions[<?php echo $key; ?>][WEIGHT]" value="<?php echo !empty($dimention['WEIGHT']) ? $dimention['WEIGHT'] : ''; ?>">
							</td>
							<td>
								<input type="text" name="weight_dimensions[<?php echo $key; ?>][XS]" value="<?php echo !empty($dimention['XS']) ? $dimention['XS'] : ''; ?>">
							</td>
							<td>
								<input type="text" name="weight_dimensions[<?php echo $key; ?>][S]" value="<?php echo !empty($dimention['S']) ? $dimention['S'] : ''; ?>">
							</td>
							<td>
								<input type="text" name="weight_dimensions[<?php echo $key; ?>][M]" value="<?php echo !empty($dimention['M']) ? $dimention['M'] : ''; ?>">
							</td>
							<td>
								<input type="text" name="weight_dimensions[<?php echo $key; ?>][L]" value="<?php echo !empty($dimention['L']) ? $dimention['L'] : ''; ?>">
							</td>
							<td>
								<input type="text" name="weight_dimensions[<?php echo $key; ?>][XL]" value="<?php echo !empty($dimention['XL']) ? $dimention['XL'] : ''; ?>">
							</td>
							<td>
					...
					<?php } else { ?>
						<tr data-row="0">
							<td>
								<input type="text" name="weight_dimensions[0][WEIGHT]">
							</td>
							<td>
								<input type="text" name="weight_dimensions[0][XS]">
							</td>
							<td>
								<input type="text" name="weight_dimensions[0][S]">
							</td>
							<td>
								<input type="text" name="weight_dimensions[0][M]">
							</td>
							<td>
								<input type="text" name="weight_dimensions[0][L]">
							</td>
							<td>
								<input type="text" name="weight_dimensions[0][XL]">
							</td>
							<td>
	...
	public function validateSpeedyForm( $admin_validation = false ) {
		...
		if ($admin_validation) {
			if ((mb_strlen($_POST['contents'], "UTF-8") < 1) || (mb_strlen($_POST['contents'], "UTF-8") > 100)) {
		...
		if (!isset($_POST['abroad']) || !$_POST['abroad']) {
			if ($_POST['postcode'] && $_POST['city'] && $_POST['city_id'] &&
				(!$_POST['to_office'] && (($_POST['quarter'] && ($_POST['quarter_id'] && $_POST['city_nomenclature'] == 'FULL' || $_POST['city_nomenclature'] != 'FULL') && ($_POST['block_no'] || $_POST['street_no'])) ||
				($_POST['street'] && ($_POST['street_id'] && $_POST['city_nomenclature'] == 'FULL' || $_POST['city_nomenclature'] != 'FULL') && ($_POST['block_no'] || $_POST['street_no'])) || $_POST['note']) || ($_POST['to_office'] && $_POST['office_id']))) {
			} else {
				if ($_POST['to_office']) {
		...
		if ( isset( $_POST['cod'] ) && $_POST['cod'] && isset( $_POST['active_currency_code'] ) ) {
			$currency_exists = false;
			foreach ( $this->currency_rate as $currency_rate ) {
				if ( $currency_rate['iso_code'] == $_POST['active_currency_code'] ) {
			...
			if ( ! $currency_exists ) {
				$errors['error_currency'] = sprintf( __( 'Не може да използвате Наложен платеж, валутата %s лиспва. Моля обърнете се към администраторите на магазина!', SPEEDY_TEXT_DOMAIN ), $_POST['active_currency_code'] );
			}
		}
	 ...
	 */
	public function getQuotePublic($address = '', $method_id) {
		global $woocommerce;
		$quote_data = array();
		...
		if (WC()->session->get( 'speedy' )) {
			...
			if ($this->pricing == 'fixed' || $this->pricing == 'calculator_fixed') {
				$speedy_session['total'] += $this->fixed_price;
			}
			...
			if (!$speedy_session['abroad']) {
				...
				$weight_size = $this->getSpeedyWeightDimention($woocommerce->cart->cart_contents_weight, $countProducts);
			...
			$speedy_session['default_methods_fixed_time'] = array();

			if ($this->fixed_time) {
				$methods_fixed_time = $this->speedy->calculate(array_merge($speedy_session, array('fixed_time' => '1030')));

				foreach ($methods_fixed_time as $method) {
					if (!$method->getErrorDescription()) {
						$speedy_session['default_methods_fixed_time'][$method->getServiceTypeId()] = $method;
					}
				}
			}
			...
			if (!$this->speedy->getError()) {
				foreach ($methods as $method) {
					...
					if ($this->fixed_time) {
						if (isset($speedy_session['fixed_time_cb'][$method->getServiceTypeId()])) {
							$method_fixed_time = $this->speedy->calculate(array_merge($speedy_session, array('fixed_time' => $speedy_session['fixed_time_hour'][$method->getServiceTypeId()] . $speedy_session['fixed_time_min'][$method->getServiceTypeId()])));
					...
					if (!$method->getErrorDescription()) {
						if ( ( $this->pricing == 'free' ) && ( $total >= (float)$this->free_shipping_total ) &&
							...
							$filter_data = array(
								...
								'weight' => $weight,
								'order_total' => $total,
								'fixed_time_delivery' => isset($speedy_session['fixed_time_cb'][$method->getServiceTypeId()]) ? $speedy_session['fixed_time_cb'][$method->getServiceTypeId()] : 0,
							);
						...
						if (isset($speedy_session['fixed_time_cb'][$method->getServiceTypeId()])) {
							$fixed_time_cb = $speedy_session['fixed_time_cb'][$method->getServiceTypeId()];
						} else {
							$fixed_time_cb = false;
						}

						if (isset($speedy_session['fixed_time_hour'][$method->getServiceTypeId()])) {
							$fixed_time_hour = $speedy_session['fixed_time_hour'][$method->getServiceTypeId()];
						} else {
							$fixed_time_hour = '10';
						}

						if (isset($speedy_session['fixed_time_min'][$method->getServiceTypeId()])) {
							$fixed_time_min = $speedy_session['fixed_time_min'][$method->getServiceTypeId()];
						} else {
							$fixed_time_min = '30';
						}
						...
						$quote_data[] = array(
							...
							'default_fixed_time' => ($this->fixed_time && isset($speedy_session['default_methods_fixed_time'][$method->getServiceTypeId()]) ? true : false),
							'error_fixed_time'   => ($fixed_time_cb && $this->fixed_time && isset($speedy_session['default_methods_fixed_time'][$method->getServiceTypeId()]) && $error_fixed_time ? true : false),
							'tax_class_id'   => 0,
				...
				// Премахване на Object - бъгва serialize
				if ( isset( $speedy_session['default_methods_fixed_time'] ) ) unset( $speedy_session['default_methods_fixed_time'] );
				WC()->session->set( 'speedy', $speedy_session);
	...
	static function speedy_before_checkout_process( ) {
		...
		if (($chosen_shipping == $speedy_shipping_method->id)) {
			if ( ! WC()->session->speedy ) {
				wc_add_notice( __( 'Моля, натиснете бутона Изчисли цена!', SPEEDY_TEXT_DOMAIN ), 'error' );
			}
	...
	static function speedy_admin_order_meta( $post ) {
		...
		if ( $shipping_method['method_id'] == $speedy_shipping_method->id ) {
			$speedy_order_info = $speedy_shipping_method->_getOrderByOrderId( $order->get_order_number() );

			if ($speedy_order_info) {
				if ($speedy_order_info['bol_id']) {
					$speedy_shipping_method->getSpeedyLoadingInfo($speedy_order_info);
				} else {
					$speedy_shipping_method->getSpeedyGenerateForm($order->get_order_number(), $speedy_order_info);
				}
			} 
		}
	}
	...
	private function getSpeedyGenerateForm($order_id, $speedy_order_info) {
		...
		if ($data['city_id']) {
			$data['offices'] = $speedy_shipping_method->speedy->getOffices( null, $data['city_id'], $lang );
	...
	static function generate_loading() {
		...
		if ( $_SERVER['REQUEST_METHOD'] == 'POST' && empty( $errors ) ) {
			...
			if ( $shipping_method_id && isset( WC()->session->shipping_method_cost[$shipping_method_id] ) && isset( WC()->session->shipping_method_title[$shipping_method_id] )) {
				...
				if ( isset( $_POST['fixed_time_cb'][$shipping_method_id] ) && isset( WC()->session->loading_methods_fixed_time_serialize[$shipping_method_id] ) ) {
					$method = maybe_unserialize( WC()->session->loading_methods_fixed_time_serialize[$shipping_method_id] );
					if ( $method->getServiceTypeId() == $shipping_method_id ) {
				...
				if ( isset( $_POST['fixed_time_cb'][$shipping_method_id] ) && isset( WC()->session->loading_methods_fixed_time_serialize[$shipping_method_id] ) ) {
					$_POST['fixed_time'] = $_POST['fixed_time_hour'][$shipping_method_id].$_POST['fixed_time_min'][$shipping_method_id];
				} else {
			...
			if (isset( $_POST['shipping_method_id'] ) && !isset($_POST['fixed_time_cb']) && isset($_POST['fixed_time_min']) && isset($_POST['fixed_time_hour'])) {
				unset($speedy_order_data['fixed_time_cb']);
			}

			if (!isset( $_POST['shipping_method_id'] ) && isset($speedy_order_data['shipping_method_id']) && !isset($speedy_order_data['fixed_time_cb'][$speedy_order_data['shipping_method_id']]) && isset($speedy_order_data['fixed_time_min']) && isset($speedy_order_data['fixed_time_hour'])) {
		...
		if ( isset( $_POST['order_id'] ) ) {
			...
			foreach($order->get_items() as $order_item) {
				...
				if ( ! $_product->is_downloadable()) {
					$product_weight = (float)$_product->get_weight();
					if (!empty($product_weight)) {
						$weight += $product_weight * $order_item['qty'];
					} else {
						$weight += $speedy_shipping_method->default_weight * $order_item['qty'];
					}
				} else {
					$totalNoShipping += $_product->get_price_including_tax($order_item['qty']);
				}
			}
	...
	public function getQuoteAdmin() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			$loading_methods_fixed_time = array();
			...
			if ( $this->fixed_time ) {
				...
				foreach ($methods_fixed_time as $method) {
					if (!$method->getErrorDescription()) {
						$loading_methods_fixed_time[$method->getServiceTypeId()] = $method;
						$loading_methods_fixed_time_serialize[$method->getServiceTypeId()] = maybe_serialize( $method );
					}
				}
			}
			...
			if (!$this->speedy->getError()) {
				foreach ($methods as $method) {
					$error_fixed_time = true;
					if ( $this->fixed_time ) {
						if (!isset( $_POST['shipping_method_id'] ) && isset($speedy_order_data['fixed_time_cb'][$method->getServiceTypeId()]) && isset($loading_methods_fixed_time[$method->getServiceTypeId()])) {
							$_POST['fixed_time_cb'][$method->getServiceTypeId()] = $speedy_order_data['fixed_time_cb'][$method->getServiceTypeId()];
							$_POST['fixed_time_hour'][$method->getServiceTypeId()] = $speedy_order_data['fixed_time_hour'][$method->getServiceTypeId()];
							$_POST['fixed_time_min'][$method->getServiceTypeId()] = $speedy_order_data['fixed_time_min'][$method->getServiceTypeId()];
						}

						if (isset($_POST['fixed_time_cb'][$method->getServiceTypeId()])) {
							$_POST['fixed_time'] = $_POST['fixed_time_hour'][$method->getServiceTypeId()] . $_POST['fixed_time_min'][$method->getServiceTypeId()];
					...
					if (!$method->getErrorDescription()) {
						...
						if (isset($_POST['fixed_time_cb'][$method->getServiceTypeId()])) {
							$fixed_time_cb = $_POST['fixed_time_cb'][$method->getServiceTypeId()];
						} elseif ($_SERVER['REQUEST_METHOD'] != 'POST' && isset($speedy_order_data['fixed_time_cb'][$method->getServiceTypeId()])) {
							$fixed_time_cb = $speedy_order_data['fixed_time_cb'][$method->getServiceTypeId()];
						} else {
							$fixed_time_cb = false;
						}

						if (isset($_POST['fixed_time_hour'][$method->getServiceTypeId()])) {
							$fixed_time_hour = $_POST['fixed_time_hour'][$method->getServiceTypeId()];
						} elseif (isset($speedy_order_data['fixed_time_hour'][$method->getServiceTypeId()])) {
							$fixed_time_hour = $speedy_order_data['fixed_time_hour'][$method->getServiceTypeId()];
						} else {
							$fixed_time_hour = '10';
						}

						if (isset($_POST['fixed_time_min'][$method->getServiceTypeId()])) {
							$fixed_time_min = $_POST['fixed_time_min'][$method->getServiceTypeId()];
						} elseif (isset($speedy_order_data['fixed_time_min'][$method->getServiceTypeId()])) {
							$fixed_time_min = $speedy_order_data['fixed_time_min'][$method->getServiceTypeId()];
						} else {
							$fixed_time_min = '30';
						}
						...
						$quote_data[] = array(
							...
							'fixed_time_hour'=> $fixed_time_hour,
							'fixed_time_min' => $fixed_time_min,
							'default_fixed_time' => ($this->fixed_time && isset($loading_methods_fixed_time[$method->getServiceTypeId()]) ? true : false),
							'error_fixed_time'   => ($fixed_time_cb && $this->fixed_time && isset($loading_methods_fixed_time[$method->getServiceTypeId()]) && $error_fixed_time ? true : false),
							'tax_class_id'   => 0,
				...
				WC()->session->loading_methods_fixed_time_serialize = isset( $loading_methods_fixed_time_serialize ) ? $loading_methods_fixed_time_serialize : array();
