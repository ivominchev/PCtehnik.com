...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.4.0', __( 'WooCommerce Order Status Manager', 'woocommerce-order-status-manager' ), __FILE__, 'init_woocommerce_order_status_manager', array(
...
class WC_Order_Status_Manager extends SV_WC_Plugin {
	...
	protected function upgrade( $installed_version ) {
		...
		if ( version_compare( $installed_version, '1.6.0', '<' ) ) {
			...
			foreach ( $this->order_statuses->get_core_order_statuses() as $slug => $core_status ) {
				...
				// move on if this status already has "is paid" meta saved
				// that means it was set to "yes" previously or via edit
				...
				switch ( $slug ) {
					...
					case 'pending':
					case 'failed':
						add_post_meta( $post_id, '_is_paid', 'needs_payment', true );
					break;
	...
	public function __get( $property ) {
		...
		$main_class = 'wc_order_status_manager()';
		$deprecated = $main_class . '->' . $property;

		switch ( $property ) {

			case 'admin' :
				_deprecated_function( $deprecated, '1.5.0', $main_class . '->get_admin_instance()' );
				return $this->get_admin_instance();

			case 'ajax' :
				_deprecated_function( $deprecated, '1.5.0', $main_class . '->get_ajax_instance()' );
				return $this->get_ajax_instance();

			case 'emails' :
				_deprecated_function( $deprecated, '1.5.0', $main_class . '->get_emails_instance()' );
				return $this->get_emails_instance();

			case 'frontend' :
				_deprecated_function( $deprecated, '1.5.0', $main_class . '->get_frontend_instance()' );
				return $this->get_frontend_instance();

			case 'icons' :
				_deprecated_function( $deprecated, '1.5.0', $main_class . '->get_icons_instance()' );
				return $this->get_icons_instance();

			case 'order_statuses' :
				_deprecated_function( $deprecated, '1.5.0', $main_class . '->get_order_statuses_instance()' );
				return $this->get_order_statuses_instance();
